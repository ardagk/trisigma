...
class RabbitMQHandler(logging.Handler):
    ...
    def __init__(self, level=logging.NOTSET,
        ...
        # :param host:                  RabbitMQ host. Default localho)st
        ...
        self.connection_params.update(dict(host=host, port=port, heartbeat=heartbeat, blocked_connection_timeout=150)) # type: ignore

        if username and password:
            self.connection_params['credentials'] = credentials.PlainCredentials(username, password) # type: ignore
    ...
    def open_connection(self):
        ...
        if not self.connection or self.connection.is_closed:
            self.connection = pika.BlockingConnection(pika.ConnectionParameters(**self.connection_params)) # type: ignore

        if not self.channel or self.channel.is_closed:
            self.channel = self.connection.channel()

        if self.exchange_declared is False:
            self.channel.exchange_declare(exchange=self.exchange, exchange_type='topic', durable=True, auto_delete=False) # type: ignore
    ...
    def emit(self, record):
        ...
        try:
            ...
            if hasattr(record, 'request'):
                ...
                del no_exc_record.request # type: ignore
                ...
                if ExceptionReporter:
                    reporter = ExceptionReporter(record.request, is_email=False, *exc_info) # type: ignore
